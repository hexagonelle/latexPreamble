...
\setmathfont{OpenDyslexic}[range={"01-"0250,"0252-"27,"30-"7B,"7D-"FF,"2B,"2D,"2E,"B7}]
\setmathfont{KPMath-Bold}[range={"1D736,"1D6FC}]

%The standard alpha character is hard to distinguish from "a" in OpenDyslexic. Replace it here:
\newcommand{\calpha}{\symbol{"1D736}}
...
% Screw threads
\tikzset{screw/.pic={	
	\fill[fill=black](-0.125,0)rectangle(0.125,0.8);
	\foreach \x in {1,...,3}{
		\fill[fill=black,rotate around = {35:(0,\x/5)}]
			($(0,\x/5) + (0.25,0)$)arc(0:180:0.25 and 0.025);
		\fill[fill=black,rotate around = {35:(0,\x/5)}]
			($(0,\x/5) + (-0.25,0)$)arc(180:360:0.25 and 0.025);
		\draw[draw=white,rotate around = {35:(0,\x/5)}]
			($(0,\x/5) + (0.25,0)$)arc(0:180:0.25 and 0.025);
	}
}}

\tikzset{hookLower/.pic={
	\draw[line width = 5]
		($(0,-0.465)+(0.315,0)$)arc(360:110:0.3);
	\draw[line width = 5]
		($
			({0.3*cos(115)},{0.3*sin(115)})+
			(0.015,-0.465)
		$)arc(295:362:0.2);
}}
...
% Adds a curly theta symbol to compensate for unicode-math overwriting amsmath's /vartheta
...
% Shorthand for writing scientific notation with units:
\newcommand{\qtysci}[3]{\qty{#1e#2}{#3}}
